...
void philosopher(int myrank)
{
    ...
    while (true)
    {
        ...
        // Request forks
        MPI_Recv(in_buffer, 1, MPI_INT, 0, FORK_RESPONSE, MPI_COMM_WORLD,
                 &stat); // Whait for response
        if (DEBUG)
...
void table(int myrank, int nprocs)
{
    ...
    // Table main loop while(true){
    MPI_Recv(in_buffer, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG,
             MPI_COMM_WORLD, &stat); // Recive next message
    ...
    { // If Request for forks
        ...
        {
            // If both forks are free fork[philosopher%(nprocs-1)] = false; //Set the forks as taken
            fork[philosopher - 1] = false;
            MPI_Send(out_buffer, 1, MPI_REAL, philosopher,
                     FORK_RESPONSE, MPI_COMM_WORLD); // Send Fork response to the
            right philosopher if (DEBUG)
                printf("Table sent philosopher %d the forks\n", philosopher);
        }
    }
    else // If not both forks are free queue.push_back(philosopher); //Put in wait queue
        ...
        {                                            // If Release of forks
            ...
            if (!queue.empty())
            { // If philosopher whaiting for forks
                for (std::list<int>::iterator it = queue.begin(); it != queue.end();
                     it++)
                { // Go through whole list of whaiting philosophers philosopher = *it;
                    if (fork[philosopher % (nprocs - 1)] == true && fork[philosopher - 1] == true)
                    { // If one of them can get both forks fork[philosopher%(nprocs1)] = false;
                        fork[philosopher - 1] = false;
