...
int main(int argc, char** argv) {
	...
	//init MPI
    MPI_Init(&argc, &argv);
    MPI_Comm_size(MPI_COMM_WORLD, &nprocs); // total no of processes in specified communicator - mpi_comm_world
    MPI_Comm_rank(MPI_COMM_WORLD, &myrank); // get rank of the calling process
...
void philosopher(int myrank){
	...
	int in_buffer[1];
	int out_buffer[1];
	MPI_Status stat; // status of completed MPI operations
	out_buffer[0];
...
/* 
  Compile:
    mpic++ dining_philosopher.cpp -o dining_philosopher

  Execute:
    mpiexec -np 6 ./dining_philosopher
...
MPI_SOURCE: The rank of the source process in a communication operation. 
It indicates which process sent the message.

MPI_TAG: The tag associated with a message. 
It provides a way to label or identify different types of messages.

MPI_ERROR: An error code associated with a communication operation. 
A non-zero value indicates an error.

When you perform an MPI communication operation, you can pass an 
MPI_Status object as an argument to functions like MPI_Recv to obtain information 
about the completed communication. 
*/
