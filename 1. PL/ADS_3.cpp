#include<iostream>
//#include<cstdio>
//#include<sstream>
//#include<algorithm>
//#define pow2(n) (1 << (n))
using namespace std;
class avl {
   public:
   int d;
   avl *l;
   avl *r;
}*r;
class avl_tree{
public:
	int height(avl *);
	int difference(avl *);
	avl *rr_rotat(avl *);
	avl *ll_rotat(avl *);
	avl *lr_rotat(avl*);
	avl *rl_rotat(avl *);
	avl * balance(avl *);
	avl * insert(avl*, int);
	void show(avl*, int);
	void inorder(avl *);
	void preorder(avl *);
	void postorder(avl*);
	avl_tree(){
		r = NULL;
	}
};
int avl_tree::height(avl *t){ // Calculates the height o current Node
   	int h = 0;
   	if (t != NULL){
		int l_height = height(t->l);
		int r_height = height(t->r);
		int max_height = max(l_height, r_height);
		h = max_height + 1;
   	}
   	return h;
}
int avl_tree::difference(avl *t){  // Calculates the Balance factor at Current Node.
   int l_height = height(t->l);
   int r_height = height(t->r);
   int b_factor = l_height - r_height;
   return b_factor;
}
avl *avl_tree::rr_rotat(avl *parent){// Left Rotation
   avl *t;
   t = parent->r;
   parent->r = t->l;
   t->l = parent;
   cout<< endl << " Left Rotation ==> RR case";
   return t;
}
avl *avl_tree::ll_rotat(avl *parent){ // Right Rotation
   avl *t;
   t = parent->l;
   parent->l = t->r;
   t->r = parent;
   cout<< endl << " Right Rotation ==> LL Case";
   return t;
}
avl *avl_tree::lr_rotat(avl *parent){ // Left Right Rotation (LR)
   avl *t;
   t = parent->l;
   parent->l = rr_rotat(t);
   cout<< endl << " LR Rotation";
   return ll_rotat(parent);
}
avl *avl_tree::rl_rotat(avl *parent){// Right Left Rotation (RL)
   avl *t;
   t = parent->r;
   parent->r = ll_rotat(t);
   cout<< endl << " RL Rotation";
   return rr_rotat(parent);
}
avl *avl_tree::balance(avl *t){   // Apply Rotations to Balance the Node / Tree
   	int bal_factor = difference(t);
   	if(bal_factor > 1){   // Height of Left Subtree is more than right subtree.
      	if(difference(t->l) > 0)
        	t = ll_rotat(t);   // Balance factor of left child of t is +ve == Right Rotation
      	else
        	t = lr_rotat(t);   // Balance factor of left child of t is -ve == LR Rotation
   	}else if(bal_factor < -1){ // Height of Right Subtree more than left subtree
      	if(difference(t->r) > 0)
        	t = rl_rotat(t);   // Balance factor of right child of t is +ve == RL Rotation
      	else
        	t = rr_rotat(t);   // Balance actor of right child of t is -ve == Left Rotation
   	}
   	return t;
}
avl *avl_tree::insert(avl *r, int v){// Insertation in AVL Tree
  	if (r == NULL) {
		r = new avl;
		r->d = v;
		r->l = NULL;
		r->r = NULL;
		return r;
   	}else if (v < r->d) {
		r->l = insert(r->l, v);
		r = balance(r);
   	}else if (v >= r->d) {
		r->r = insert(r->r, v);
		r = balance(r);
   	}return r;
}
void avl_tree::show(avl *p, int l){ // Display of AVL Tree
   int i;
   if (p != NULL){
      	show(p->r, l+ 1);
      	cout<<" ";
      	if (p == r)
        	cout << "Root -> ";
      	for(i = 0; i < l&& p != r; i++)
        	cout << " ";
        	cout << p->d;
        	show(p->l, l + 1);
   }
}
void avl_tree::inorder(avl *t){ // Inorder Traversal
   	if(t == NULL)
	    return;
	inorder(t->l);
	cout << t->d << " ";
	inorder(t->r);
}
void avl_tree::preorder(avl *t){// Preorder Traversal
   	if (t == NULL)
    	return;
	cout << t->d << " ";
	preorder(t->l);
	preorder(t->r);
}
void avl_tree::postorder(avl *t) {// Postorder Traversal
   	if (t == NULL)
        return;
    postorder(t ->l);
    postorder(t ->r);
    cout << t->d << " ";
} 
int main(){
	int c, i;
	avl_tree avl;
	while(1){
	cout <<"\n Operations on AVL Tree: " << endl;
	cout <<"1. Construct AVL" << endl;
	cout <<"2. Insert Element into the tree" << endl;
	cout <<"3. Show Balanced AVL Tree" << endl;
	cout <<"4. InOrder Traversal" << endl;
	cout <<"5. PreOrder Traversal" << endl;
	cout <<"6. PostOrder Traversal" << endl;
	cout <<"7. Exit" << endl;
	cout <<"Enter your Choice: ";
	cin >> c;
	switch (c){
        case 1: int t;            //Number of nodes to be inserted
				cout<<endl<<"Enter number of nodes to insert in AVL Tree :- ";
				cin>>t;
				while(t--){
				    int d;
				    cout<<endl<<"Enter "<<t<<" Element:- ";
				    cin>>d;
				    r = avl.insert(r,d);
				}
				cout<<"\n Constructed"<<endl;
				break;
        case 2: cout << "\n Enter value to be inserted: ";
				cin >> i;
				r = avl.insert(r, i);
				break;
        case 3: if (r == NULL) {
					cout <<"Tree is Empty! " << endl;
					continue;
				}
				cout <<"\n Balanced AVL Tree:" << endl;
				avl.show(r, 1);
				cout<<endl;
				break;
        case 4: if (r == NULL){
					cout <<"Tree is Empty! " << endl;
					continue;
				}
				cout <<"\n Inorder Traversal: ";
				avl.inorder(r);
				cout << endl;
				break;
        case 5: if (r == NULL) {
					cout <<"Tree is Empty! " << endl;
					continue;
				}
				cout <<"\n Preorder Traversal: ";
				avl.preorder(r);
				cout << endl;
				break;
        case 6: if (r == NULL) {
					cout <<"Tree is Empty! " << endl;
					continue;
				}
				cout <<"\n Postorder Traversal: ";
				avl.postorder(r);
				cout << endl;
				break;
        case 7: exit(1);
				break;
        default:cout <<"Wrong Choice! " << endl;
      }
   }
   return 0;
}
